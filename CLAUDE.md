...
> **⚠️ Важно:** Все изменения вносятся в основной репозиторий `D:\vue\task-flow`, а не в worktree.
...
- **PrimeVue 4** - UI-библиотека (все новые UI-компоненты берутся отсюда)
- **Lucide** - иконки
- **Vitest** - тестирование
...
1. **UI-компоненты:** Все элементы форм (input, select, textarea, datepicker, button) и другие UI-компоненты брать из **PrimeVue 4**. Не использовать нативные HTML-контролы. Иконки — из **Lucide** (через `AppIcon`).
